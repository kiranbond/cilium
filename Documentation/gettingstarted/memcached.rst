...
**NOTE:** memcached-aware policy support is still "experimental".  It is not yet ready for
...
- **memcached-server** : A k8s service represented by a single pod running a memcached server (label app=memcd-server).
...
.. parsed-literal::
   ...
   $  curl -s \ |SCM_WEB|\/examples/memcd-env.sh | source
...
Thus, the Alliance Fleet Tracking System could be made weak if an single starfighter ship is compromised.
